...
class TableController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($restaurant_id != "") 
        {
            ...
            if(!empty($table_details))
            {
                ...
                {
                    // Validate Table Number
                    $validate_table_number = TableModel::where('table_number',$request->table_number)->where('restaurant_id',$restaurant_id)->where('table_id','!=',$request->table_id)->first();
                        // echo "<pre>$request->table_id"; print_r($validate_table_number); echo "</pre>"; exit();
                    if (empty($validate_table_number))
                    {
                        $full_qr_codes_text = '';
    ...
    //             if(!empty($tableData['table_number']) && !empty($tableData['restaurant_id']) && !empty($tableData['chairs']) && $restaurant_id == $tableData['restaurant_id']){    
    //                 $validate_table_number = TableModel::where('table_number',$tableData['table_number'])->where('restaurant_id',$tableData['restaurant_id'])->count();
    //                 if ($validate_table_number ==0){
    //                     $full_qr_code_image_name = '';
    //                     $qr_code_image_name = '';
    //                     if ($tableData['chairs'] > '0'){
    //                         for ($i=1; $i <= $tableData['chairs']; $i++){ 
    //                             $chair_number = $i;
    //                             $directory = config('images.qr_code_url').$tableData['restaurant_id'];
    //                             // if(!File::exists($directory)) 
    //                             // {
    //                             //     File::makeDirectory($directory);
    //                             // }
    //                             $qr_code_image_name = "qrcode_".$tableData['table_number']."_".$chair_number.".png";
    //                             // $qr_path = $directory."/".$qr_code_image_name;
    //                             //$qr_image = TableModel::generate_qr_code($qr_path, $restaurant_id);
    //                             $full_qr_code_image_name .= $qr_code_image_name." ";
    //                         }
    ...
    //                     // Add row
    //                     $table_data= new TableModel();
    //                     $table_data->restaurant_id = $tableData['restaurant_id'];
    //                     $table_data->table_number = $tableData['table_number']; 
    //                     $table_data->chairs      = $tableData['chairs'];
    //                     //$table_data->qr_code      = trim($full_qr_code_image_name);
    //                     $table_data->qr_code      = $full_qr_code_image_name;
    //                     $table_data->created_at    = date('Y-m-d H:i:s');
    //                     $row_added = $table_data->save();
    //                 }  
    //             }
    //         }
    //          return response()->json(['success'=>'Data Import successfully']);
    //     }else{
    //         return response()->json(['error'=>$validator->errors()]);
    //     }
    // }
    public function import(Request $request){
        ...
        if ($validator->passes()){
            ...
            foreach($data as $tableData){
                if(!empty($tableData['table_number']) && !empty($restaurant_id) && !empty($tableData['chairs'])){    
                    $validate_table_number = TableModel::where('table_number',$tableData['table_number'])->where('restaurant_id',$restaurant_id)->count(); 
                    ...
                    if ($validate_table_number ==0){ 
                        ...
                        if ($tableData['chairs'] > '0'){
                            for ($i=1; $i <= $tableData['chairs']; $i++){ 
                                ...
                                $full_qr_code_image_name = "qrcode_".$tableData['table_number']."_".$chair_number.".png";
                                $qr_path = $directory."/".$full_qr_code_image_name;
                                $qr_image = TableModel::generate_qr_code($qr_path, $restaurant_id,$tableData['table_number'],$chair_number);
                                $full_qr_codes_text .= $full_qr_code_image_name." ";
                            }
                        }
                        // Add row
                        $table_data= new TableModel();
                        ...
                        $table_data->table_number = $tableData['table_number']; 
                        $table_data->chairs      = $tableData['chairs'];
                        $table_data->qr_code      = $full_qr_codes_text;
                        $table_data->created_at    = date('Y-m-d H:i:s');
                        $row_added = $table_data->save();
                    }  
                }
