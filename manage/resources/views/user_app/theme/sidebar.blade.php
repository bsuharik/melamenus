...
?>
        <div class="panel-control-left">
            <ul id="slide-out-left" class="section-title-new side-nav side-nav-img collapsible" data-collapsible="accordion">
                        ...
                        @foreach($parent_categories as $parent_cat)
                            <li>
                                <div class="collapsible-header">
                                   ...
                                   <p class='section-title-new' id="sidebar_category">{{ $parent_cat->category_name}} </p> <span><i class="fa fa-chevron-right"></i></span>
                                </div>
                                <div class="collapsible-body">
                                    ...
                                    @if(count($main_categories) > '0')
                                        @foreach($main_categories as $main_cat)
                                            @if($main_cat->parent_category_id == $parent_cat->category_id)
                                                <li>
                                                    <div class="section-title-new collapsible-header">
                                                    @if(count($sub_categories) > '0')
                                                        ...
                                                        @else
                                                            <a href="{{ url('sub_categories') }}/{{ $main_cat->category_id }}"  class="section-title-new" tyle="padding: 0px;">
                                                                @if(!empty($main_cat->category_icon))
                                                                <img src="../{{config('images.category_url') .$restaurant_details->restaurant_id.'/'.$main_cat->category_icon}}" alt="img">
                                                                @else
                                                                <img src="../{{config('images.default_image_url')}}" height="40" width="40" alt="img">
                                                        ...
                                                        @endif
                                                    @else
                                                        <a href="{{ url('sub_categories') }}/{{ $main_cat->category_id }}" style="padding: 0px;" class="section-title-new">
                                                            @if(!empty($main_cat->category_icon))
                                                            <img src="../{{config('images.category_url') .$restaurant_details->restaurant_id.'/'.$main_cat->category_icon}}" alt="img">
                                                            @else
                                                            <img src="../{{config('images.default_image_url')}}" height="40" width="40" alt="img">
                                                ...
                                                @if(count($sub_categories) > '0')
                                                    @foreach($sub_categories as $sub_cat)
                                                        @if($sub_cat->main_category_id == $main_cat->category_id)
                                                            <li>
                                                                <p  class='section-title-new' id="sidebar_category">
                                                                    <a href="{{ url('menu_items') }}/{{ $sub_cat->category_id }}" class="section-title-new"> 
