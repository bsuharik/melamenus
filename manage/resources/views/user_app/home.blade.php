    ...
    <div class="slider-entry"> 
                ...
                @else
                    @if(!empty($parent_restaurant_details->restaurant_cover_image))
                        <img src="../{{ config('images.restaurant_url') .$parent_restaurant_details->restaurant_id.'/'. $parent_restaurant_details->restaurant_cover_image}}" style="top: {{$restaurant_details->cover_img_position}};">
                    @else
                                ...
                                <div class="col-md-12">
                                    <a href="{{ url('main_categories') }}/{{ $parent_cat->category_id }}">
                                        <div class="entry">
                                           ...
                                           @endif
                                            <h5 class='section-title-new'> {{ $parent_cat->category_name }} </h5> 
                                        </div>
                                    </a>
                                </div>
                            @endforeach
