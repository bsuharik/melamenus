...
<div class="content-body">
    ...
    <div class="container-fluid">
        ...
        <input type="hidden" value="{{asset('uploads/images/qr_codes')}}" id="qrcode_url">
...
<script src="{!! asset('theme/admin/js/image_create.js') !!}"></script>
<!-- <script src="http://localhost/image_create.js"></script> -->
...
$(document).ready(function (e) {
                    ...
                    {   
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                qr_codes_list_html += ' <div class="row"><div class="col-sm-3 pt-3">\<label style="font-size: 24px;font-weight: 500;width: 100%;">'+$table_number+':'+i+'</label>\</div>\<div class="col-sm-6 mx-auto">\<img src="../'+$qr_code_url+$restaurant_id+'/'+$qr_codes_array[i-1]+'" height="100" width="100">\</div>\<div class="col-sm-3 pt-3">\<a href="javascript:void(0)" class="qrcode_dowload" data-table_no='+$table_number+' data-chair='+i+' data-qr_code="'+$restaurant_id+'/'+$qr_codes_array[i-1]+'">\<i class="flaticon-381-download" style="color: #6742a8;font-size: 19px;font-weight: 500;"></i></a>\</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\<a href="#" data-id="../'+$qr_code_url+$restaurant_id+'/'+$qr_codes_array[i-1]+'" data-table="'+$table_number+'" data-chairs="'+i+'" class="print_qr_code">\<i class="flaticon-381-print"  style="color: #6742a8;font-size: 19px;font-weight: 500;"></i>\</a>\ </div></div>';
                                }
                            }
                        }
    ...
    // import table CSV file 
    //import table
    $('#import_table').submit(function(e) {
        e.preventDefault();
        var formData = new FormData(this);
         //console.log(formData);
        $.ajax({
            type:'POST',
            url: "{{ url('import_table')}}",
            data: formData,
            cache:false,
            contentType: false,
            processData: false,
...
$(document).ready(function () {
    // // Downloadv qr code image  
    $(document).delegate('.qrcode_dowload', 'click', function (event) {
        var image_url = $("#image_url").val();
        var table_number = $(this).attr("data-table_no");
        var chairs = $(this).attr("data-chair");
        ...
        $('#myHtml').html(html);
        $('#myHtml').show();
        var html_content=document.getElementById("myHtml");
         // console.log(html_content); return false;
