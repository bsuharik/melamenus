            ...
            <div class="row">
                ...
                <div class=" col-md-6"> 
                    <div class="card">
                        ...
                        <div class="card-body">
                            <label><b>Menu Display Image </b></label>
                            <div class=" col-md-12">
                                <div class="row">
                                    @if($menu_detail->menu_image != "")
                                        <img src="../{{ config('images.menu_url') .$menu_detail->restaurant_id.'/'. $menu_detail->menu_image }}" width="40%"/>
                                        @endif 
                                        <div id="errorMessage"></div>
                                       <input type="file" name="menu_image" id="menu_image"> 
                                       <div class="col-md-1">
                                    <button id="menu_image_add" type="button" class="btn btn-primary mb-2" style="float:right;">&#x2B;</button>
                                    </div>
                            ...
                            <br>
                            <label><b>Menu Other Image </b></label>
                            @if($menu_detail->menu_other_image != "")
                            <?php
                                $otherImageArray= explode(",",$menu_detail->menu_other_image);
                                $i=1;
                            ?>

                            @foreach($otherImageArray as $otherImage)
                                <div class=" col-md-12 count_menu_other_image">
                                    <div class="row">
                                        <img src="../{{ config('images.menu_url') .$menu_detail->restaurant_id.'/'.$otherImage }}" width="40%"/>
                                        <input type="file" name="menu_other_image[]">
                                        <div id="errorMessage"></div>
                                        
                                        <input type="hidden" name="old_menu_other_image[]" value="{{$otherImage}}" class="old_menu_other_image">
                                        
                                    </div>
                                    <div class="col-md-1">
                                    <button id="remove_field_image" type="button" class="btn btn-primary" data-toggle="modal" data-target="#deleteImageModal" data-class="remove_field_image{{$i}}" data-image_type="1"style="float:right;">-</button>
                                </div>
                                </div>
...
<script type="text/javascript">
    $(document).on('change','input[type="file"]',function(){
        ...
        if ($.inArray(files['name'].split('.').pop().toLowerCase(), ['gif','png','jpg','jpeg']) == -1){
            ...
            // remove old change image
            $(this).parent().find('.menu_change_image').remove();
            var change_image= $(this).parent().find('.old_menu_other_image').val();
            $(this).parent().append('<input type="hidden" name="menu_change_image[]" value="'+change_image+'" class="menu_change_image">')
...
$(document).ready(function (e) {
    ...
    var menu_max_fields = 4; //maximum input boxes allowed
    var menu_image_wrapper    = $("#cloned_menu_image"); //Fields menu_image_wrapper
    var menu_image_add_button = $("#menu_image_add"); //Add button ID
    
    var menu_x = 0;
    //initlal text box count
    $(menu_image_add_button).click(function(e){ //on add input button click
        $('.imageErrorMessage').html(" ");
        e.preventDefault();
        $("#other_display_image").show();
        var image_count= $(".count_menu_other_image").length
        menu_x= image_count + 1;
        if(image_count < menu_max_fields){ 
            //max input box allowed
            $(menu_image_wrapper).append('<div class="col-sm-12 count_menu_other_image">\
                                                <div class="element error_message_parent">\
                                                    <input type="file" class="form-control menu_other_image" name="menu_other_image[]">\
                                                </div>\
                                                <div id="errorMessage"></div>\
                                                <div class="col-md-1">\
                                                    <button id="remove_field_image" type="button" class="btn btn-primary" data-toggle="modal" data-target="#deleteImageModal" data-class="remove_field_image'+menu_x+'" data-image_type="0"style="float:right;">-</button>\
                                                </div>\
                                            </div>\<br/>'); //add input box
            menu_x++; //text box increment
        }else{
            var html_image_errpr='<div class="alert alert-danger background-danger">\
                                        <button type="button" class="close" data-dismiss="alert" aria-label="Close">\
                                            <i class="icofont icofont-close-line-circled text-white"></i>\
                                        </button>\
                                        <strong>Error!</strong> Menu Image Maximum 5 image uploade\
                                    </div>';
            $('.imageErrorMessage').html(html_image_errpr);
        }
    });

    $('body').delegate('#remove_field_image','click', function(e){
        e.preventDefault(); 
        $('.imageErrorMessage').html(" ");
        $('#deleteImageModal').modal('show');
        $delete_image_type = $(this).attr('data-image_type');
        $delete_element_image = $(this).attr('data-class');
        $('#delete_element_image').val($delete_element_image);
        $('#delete_element_image').attr("data-img_type",$delete_image_type); //setter
        
    });

    $("#confirmDeleteImage").click( function(e){ //user click on remove text
        e.preventDefault();
        var total_other_image= $(".count_menu_other_image").length;
        if(total_other_image ==1){
            $("#other_display_image").hide();
        }
        $delete_element_image = $('#delete_element_image').val();
        $element_image_type = $('#delete_element_image').data('img_type');
        $('button[data-class='+$delete_element_image+']').parent('div').parent('div').remove();
            menu_x--;
    ...
    $(document).ready(function() {
        // Strat Multiple Tag Select 
        var tag_select = $('#tag_id');
        var tag_options = tag_select.find('option');
        var tag_div = $('<div />').addClass('selectMultiple');
        var tag_active = $('<div />');
        var tag_list = $('<ul />');
        var tag_placeholder = tag_select.data('placeholder');
        var tag_span = $('<span />').text(tag_placeholder).appendTo(tag_active);
        tag_options.each(function() {
            var tag_text = $(this).text();
            if($(this).is(':selected')) {
                tag_active.append($('<a />').html('<em>' + tag_text + '</em><i></i>'));
                tag_span.addClass('hide');
            } else {
                tag_list.append($('<li />').html(tag_text));
            }
        });
        tag_active.append($('<div />').addClass('arrow'));
        tag_div.append(tag_active).append(tag_list);
        tag_select.wrap(tag_div);
        // END Multiple Tag Select 

        // Strat Multiple Allergies Select 
