...
class TableModel extends Model
{
    ...
    // Update table
    public static function update_table_details($condition,$update_data = array())
    {
        $row_updated = DB::table('restaurant_tables')->where($condition)->update($update_data);
    ...
    public static function generate_qr_code($qr_path, $restaurant_id,$table_number,$chair_number)
    {
        ...
        $qr_image = QrCode::format('png')->size(300)->errorCorrection('H')->merge($logo_url)
                    ->generate($path,public_path($qr_path));
        return 1;
    }
